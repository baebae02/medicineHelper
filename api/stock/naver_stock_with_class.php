...
// 어제 23시 59분 00초 기준, 밀리초 기준
function getUnixTime() {
    $today = new DateTime(null, new DateTimeZone('Asia/Seoul'));   // 오늘의 Unix time
    $today = $today->setTime(23, 59, 00);
    $interval = new DateInterval('P1D');
    $today = $today->sub($interval);
    $yesterdayMidnightTimestamp = $today->getTimestamp() * 1000;
    // $yesterdayPrice = strtotime($today->format("Y-m-d 14:59:59"));
    return $yesterdayMidnightTimestamp;
}

class Stock {
    public string $code;                // 종목번호
    ...
    function __construct($code, $name, $nowPrice, $compareWithYesterday, $changeRatio, $bid, $ask, $volume, $tradingValue, $yesterdayVolume) {
        $this->code = $code;
        $this->name = $name;
    ...
    function getImageOfStock() {
        $unixTimestamp = getUnixTime();
        file_put_contents("./stockImage/$this->code.jpg", file_get_contents("https://ssl.pstatic.net/imgfinance/chart/item/area/day/$this->code.png?sidcode=$unixTimestamp"));
    }
}
...
for($i = 0; $i < count($stockList) - 2; $i++) {
    ...
    $classStock = new Stock(
        // 종목코드 : /item/main.naver?code=000225 의 마지막 "=" 이후 부분을 따왔습니다.
        (string)preg_split("/[=]+/u", $all_td_tags->item(0)->childNodes->item(0)->childNodes->item(0)->attributes->item(0)->nodeValue)[1],
